...
import {clientError} from 'redux-websocket/lib/rpc/server'
import {actions} from 'raxa-common/lib/actions'
import {Call, Device, Modification} from 'raxa-common/lib/entities'
import {Devices as DevicesRpc} from 'raxa-common/lib/remote-procedures/devices'
import {plugins} from '../services/plugin'
import {remoteProcedures} from '../services/websocket'
import {store, dispatch} from '../services/store'

@remoteProcedures()
class Devices extends DevicesRpc {
  async createDevice(device: Device) {
    ...
    if (!deviceClass) throw clientError('Invalid Device Class')
    ...
    dispatch(actions.deviceAdded, {device, interfaces})
  }
